...
class vit_models(nn.Module):
    ...
    def __init__(self, img_size=224,  patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        tmp_block = []
        for i in range(depth):
            if i in self.done_layer:
                tmp_block.append(Block_shrink( 
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                 drop=0.0, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer,
                act_layer=act_layer,Attention_block=Attention_block,Mlp_block=Mlp_block,init_values=init_scale))
            else:
                tmp_block.append( block_layers(
                 dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                 drop=0.0, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer,
                 act_layer=act_layer,Attention_block=Attention_block,Mlp_block=Mlp_block,init_values=init_scale))
        self.blocks = nn.ModuleList(tmp_block)
        ...
        print(f'the pruning layer is {self.prune_layer}, the pruned layer is {self.done_layer}')
        self.tag =[]
        for i in self.done_layer:
            if self.tag == []:
                self.tag=f'{i}'
            else:
                self.tag+=f'{i}'
    ...
    def forward(self, x, epoch=0):
        ...
        if False: # Entropy at last block
            if not hasattr(self, 'trans_entropy'):
                self.trans_entropy=[]
            self.trans_entropy.append(x.detach().cpu().numpy().astype(np.float64)) 
            if len(self.trans_entropy)>=130:
                all_logit = np.concatenate(self.trans_entropy)
                print(f'block entropy for {self.done_layer} is {np.log(np.std(all_logit,axis=0)).sum()}') 

        if self.dropout_rate:
            x = F.dropout(x, p=float(self.dropout_rate), training=self.training)
        x = self.head(x)

        if False: # Entropy at class head
            if not hasattr(self, 'trans_entropy_head'):
                self.trans_entropy_head=[]
            self.trans_entropy_head.append(x.detach().cpu()) 
            if len(self.trans_entropy_head)>=130:
                all_logit = torch.cat(self.trans_entropy_head)
                print(f'head entropy for {self.done_layer} is {torch.log2(torch.std(all_logit.float(),0)).sum()}')
        return x

#--------------------------- our models are hear ------------------

@register_model
def deit_base_patch16_224_shrink(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = vit_models(
...
@register_model
def deit_tiny_patch16_224_shrink(pretrained=False, img_size=224, pretrained_21k = False,   **kwargs):
    model = vit_models(
...
@register_model
def deit_small_patch16_224_shrink(pretrained=False, img_size=224, pretrained_21k = False,  **kwargs):
    model = vit_models(
